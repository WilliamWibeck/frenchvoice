...
import { FOCUS_OPTIONS } from "./feedback.js";
...
function formatFocus(focusId) {
  ...
  if (!note) return "";
  const meta = FOCUS_OPTIONS.find((f) => f.id === focusId);
  return `
Grammar focus for this session: ${meta ? meta.label : focusId}.
${note}
...
`;
  ...
  const focusId = FOCUS_OPTIONS.some((f) => f.id === options.focus) ? options.focus : "";
  if (focusId) {
